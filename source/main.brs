...
sub main()
    ...
    WALK_ACCEL = 1.0 : MAX_SPEED = 20 : FRICTION = 0.85
    ...
    while running
        ...
        player.velY += GRAVITY : player.y += player.velY : player.onGround = false

        for each plat in world.platforms
            if player.x + player.halfW > plat.x and player.x - player.halfW < plat.x + plat.w
                if player.y - player.velY <= plat.y and player.y >= plat.y then player.y = plat.y : player.velY = 0 : player.onGround = true
            end if
        end for
        ...
        for each plat in world.platforms
            ...
            if DEBUG_COLLISION then screen.DrawRect(screenX, plat.y, plat.w, 3, &hFF0000FF)
        end for
